...
const CharacterPopularityGraph: FC<ICharacterPopularityGraphProps> = ({ characters }) => {
	interface ICharacterPopularity {
		[key: string]: number
	}

	const targetCharactersList = ['Abradolf Lincler', 'Arcade Alien', 'Morty Smith', 'Birdperson', 'Mr. Meeseeks']

	const popularitiesByCharacterName: ICharacterPopularity | undefined = useMemo(() => {
	  if (characters.length > 0) {
	    return characters.reduce<ICharacterPopularity>((popularitiesObject: ICharacterPopularity, character: ICharacter) => {
	...
	const maxPopularityInCharacters = useMemo(() => {
	  const popularitiesArray = popularitiesByCharacterName && Object.values(popularitiesByCharacterName)
  ...
  <div>
    {maxPopularityInCharacters && popularitiesByCharacterName && Object.keys(popularitiesByCharacterName).length > 0 && (
      <div className={styles.graphContent}>

        <h2 className={styles.graphHeader}>Popularity graph of given characters</h2>
        ...
        <div className={styles.barGraph}>
          ...
          {targetCharactersList.map((characterName, index) => {
						  ...
						  const barItemHeight = Math.floor((popularity / maxPopularityInCharacters) * 100)
						  return (
  <div
    ...
    <div className={styles.barItem}>
      ...
      <dd>
        <div
          ...
          style={{ height: `${barItemHeight}px`, backgroundColor: `${color}` }}
        />
      </dd>
      <span className={styles.popularityLabel}>{popularity}
      </span>
    </div>
  </div>
						  )
